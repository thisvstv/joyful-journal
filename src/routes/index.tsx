...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: "Vault Ledger — Financial Reports" },
      { name: "description", content: "Secure client portal for financial reports and collaboration." },
    ],
  }),
});
    ...
    <div className="rounded-2xl border border-border bg-card p-6">
      ...
      <div className="mt-6 h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
            ...
            />
            <Tooltip
              contentStyle={{ borderRadius: 8, border: "1px solid oklch(0.92 0.01 250)", fontSize: 12 }}
      ...
      <div className="mt-4 flex items-center justify-center gap-6 text-xs text-muted-foreground">
        <span className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-[oklch(0.45_0.22_265)]" /> Revenue
        </span>
        <span className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-[oklch(0.78_0.08_255)]" /> Expenses
        </span>
      </div>
    </div>
  );
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Overview and collaborative analysis for Q3
              </p>
            </div>
            <div className="flex gap-3">
